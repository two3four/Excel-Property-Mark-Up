...
def app():
    ...
    if uploaded_file:
        ...
        # BULK DISCOUNT LOGIC with robust parsing
        r19_val = ws["R19"].value
        if isinstance(r19_val, (int, float)):
            for row in range(27, 55):  # D27 to D54 (data rows)
                cell_val = ws[f"D{row}"].value
                if cell_val:
                    text = str(cell_val).replace(",", "").replace("\u202f", " ").strip().lower()
                    numbers = re.findall(r"\d+", text)
                    if "and up" in text or "+" in text:
                        low = int(numbers[0])
                        high = float("inf")
                    elif len(numbers) == 2:
